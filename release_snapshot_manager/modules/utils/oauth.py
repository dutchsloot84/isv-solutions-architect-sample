...
try:
    ...
    class OAuth2Session:  # type: ignore[misc]
        ...
        def authorization_url(self, *args, **kwargs):
            raise RuntimeError("requests_oauthlib is required for the authorization flow. Install the dependency to continue.")

        def fetch_token(self, *args, **kwargs):
            raise RuntimeError("requests_oauthlib is required for the authorization flow. Install the dependency to continue.")

        def refresh_token(self, *args, **kwargs):
            raise RuntimeError("Token refresh requires requests_oauthlib. Install the dependency to continue.")
...
class OAuthCallbackHandler(BaseHTTPRequestHandler):
    ...
    def log_message(self, format: str, *args) -> None:  # noqa: A003 - method signature required
...
def authorize_jira() -> dict:
    ...
    LOGGER.info("Open the following URL in a browser to authorize access: %s", authorization_url)
...
def get_jira_session() -> OAuth2Session:
    ...
    if not verify_path:
        LOGGER.warning("REQUESTS_CA_BUNDLE is not set; SSL verification may fail.")
...
if __name__ == "__main__":
    ...
    parser.add_argument("command", choices=["authorize"], help="Run the OAuth authorization flow")
